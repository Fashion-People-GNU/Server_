...
def construct_dataset(clearml_info_string):
    """Load in a clearml yamls and fill the internal data_dict with its contents."""
    dataset_id = clearml_info_string.replace("clearml://", "")
    dataset = Dataset.get(dataset_id=dataset_id)
    dataset_root_path = Path(dataset.get_local_copy())

    # We'll search for the yaml file definition in the yamls
    yaml_filenames = list(glob.glob(str(dataset_root_path / "*.yaml")) + glob.glob(str(dataset_root_path / "*.yml")))
    if len(yaml_filenames) > 1:
        raise ValueError(
            "More than one yaml file was found in the yamls root, cannot determine which one contains "
            "the yamls definition this way."
        )
    elif not yaml_filenames:
        raise ValueError(
            "No yaml definition found in yamls root path, check that there is a correct yaml file "
            "inside the yamls root path."
        )
...
class ClearmlLogger:
    ...
    def __init__(self, opt, hyp):
        ...
        - Upload yamls version to ClearML Data if opt.upload_dataset is True
