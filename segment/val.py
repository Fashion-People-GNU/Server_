...
"""
Validate a trained YOLOv5 segment model on a segment yamls.
...
def run(
    ...
    single_cls=False,  # treat as single-class yamls
    ...
    is_coco = isinstance(data.get("val"), str) and data["val"].endswith(f"coco{os.sep}val2017.txt")  # COCO yamls
...
def parse_opt():
    """Parses command line arguments for configuring YOLOv5 options like yamls path, weights, batch size, and
    inference settings.
    """
    ...
    parser.add_argument("--single-cls", action="store_true", help="treat as single-class yamls")
